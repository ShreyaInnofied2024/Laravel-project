    ...
    <div class="login-container bg-white p-4 rounded shadow-sm w-100" style="max-width: 500px;">
        ...
        @endif
            ...
            <div class="form-group">
                ...
                <input type="number" name="quantity" id="quantity" class="form-control @error('quantity') is-invalid @enderror" value="{{ old('quantity', $product->quantity) }}" placeholder="Enter quantity">
                @error('quantity')
                    <span class="invalid-feedback">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" name="price" id="price" class="form-control @error('price') is-invalid @enderror" value="{{ old('price', $product->price) }}" placeholder="Enter price">
                @error('price')
                    <span class="invalid-feedback">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="existing-images">
                @foreach ($product->images as $image)
                    <div id="image-container-{{ $image->id }}">
                        ...
                        <!-- Image Deletion Forms -->
                        @if(auth()->user()->user_role === 'seller')
                            <form id="delete-image-{{ $image->id }}" action="{{ route('seller.products.deleteImage', $image->id) }}" method="post" style="display: inline;">
                                @csrf
                                @method('DELETE')
                                <a href="#" class="btn btn-danger btn-sm mt-2" onclick="if(confirm('Are you sure you want to delete this image?')) { event.preventDefault(); document.getElementById('delete-image-{{ $image->id }}').submit(); }">Delete</a>
                            </form>
        ...
        @elseif(auth()->user()->user_role === 'seller')
            <a href="{{ route('seller.product_admin') }}" class="btn btn-secondary mt-3">Go Back</a>
        @endif
    </div>
</section>
